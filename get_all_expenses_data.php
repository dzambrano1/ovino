...
try {
    ...
    $expenseCategories = [
        ...
        'feed' => [
            'Concentrado' => ['table' => 'oh_concentrado', 'date_field' => 'oh_concentrado_fecha_inicio', 'qty_field' => 'oh_concentrado_racion', 'price_field' => 'oh_concentrado_costo'],
            'Melaza' => ['table' => 'oh_melaza', 'date_field' => 'oh_melaza_fecha_inicio', 'qty_field' => 'oh_melaza_racion', 'price_field' => 'oh_melaza_costo'],
            'Sal' => ['table' => 'oh_sal', 'date_field' => 'oh_sal_fecha_inicio', 'qty_field' => 'oh_sal_racion', 'price_field' => 'oh_sal_costo']
        ],
        // Health/vaccine expenses (doses × cost or single cost)
        'health' => [
            'Aftosa' => ['table' => 'oh_aftosa', 'date_field' => 'oh_aftosa_fecha', 'qty_field' => 'oh_aftosa_dosis', 'price_field' => 'oh_aftosa_costo'],
            'Brucelosis' => ['table' => 'oh_brucelosis', 'date_field' => 'oh_brucelosis_fecha', 'qty_field' => 'oh_brucelosis_dosis', 'price_field' => 'oh_brucelosis_costo'],
            'Clostridiosis' => ['table' => 'oh_clostridiosis', 'date_field' => 'oh_clostridiosis_fecha', 'qty_field' => 'oh_clostridiosis_dosis', 'price_field' => 'oh_clostridiosis_costo'],
            'Neumonia' => ['table' => 'oh_neumonia', 'date_field' => 'oh_neumonia_fecha', 'qty_field' => 'oh_neumonia_dosis', 'price_field' => 'oh_neumonia_costo'],
            'Ectima' => ['table' => 'oh_ectima', 'date_field' => 'oh_ectima_fecha', 'qty_field' => 'oh_ectima_dosis', 'price_field' => 'oh_ectima_costo'],
            'Parasitos' => ['table' => 'oh_parasitos', 'date_field' => 'oh_parasitos_fecha', 'qty_field' => 'oh_parasitos_dosis', 'price_field' => 'oh_parasitos_costo'],
            'Garrapatas' => ['table' => 'oh_garrapatas', 'date_field' => 'oh_garrapatas_fecha', 'qty_field' => 'oh_garrapatas_dosis', 'price_field' => 'oh_garrapatas_costo']
        ]
    ];
        ...
        foreach ($categories as $categoryName => $config) {
            ...
            if ($categoryName === 'Concentrado' || $categoryName === 'Melaza' || $categoryName === 'Sal') {
                ...
                if ($categoryName === 'Concentrado') {
                    ...
                    $sql = "SELECT 
                                oh_concentrado_racion as racion,
                                oh_concentrado_costo as costo,
                                oh_concentrado_fecha_inicio as fecha_inicio,
                                oh_concentrado_fecha_fin as fecha_fin,
                                NULL as fecha
                            FROM oh_concentrado 
                            WHERE oh_concentrado_fecha_inicio IS NOT NULL 
                            AND oh_concentrado_fecha_fin IS NOT NULL
                            AND oh_concentrado_racion > 0 
                            AND oh_concentrado_costo > 0
                            ORDER BY oh_concentrado_fecha_inicio ASC";
                } else {
                    // Get validity period from configuration
                    $validityField = $categoryName === 'Melaza' ? 'o_vencimiento_melaza' : 'o_vencimiento_sal';
                    $configQuery = "SELECT {$validityField} FROM o_vencimiento LIMIT 1";
    ...
    foreach ($monthlyExpenses as $monthData) {
        ...
        // Format all category values to ensure proper JSON encoding
        $formattedCategories = [];
        foreach ($monthData['categories'] as $categoryName => $categoryValue) {
            $formattedCategories[$categoryName] = (float)number_format($categoryValue, 2, '.', '');
        }
        
        $data[] = [
            'month' => $monthData['month'],
            'total_expenses' => (float)number_format($monthData['total_expenses'], 2, '.', ''),
            'feed_expenses' => (float)number_format($monthData['feed_expenses'], 2, '.', ''),
            'health_expenses' => (float)number_format($monthData['health_expenses'], 2, '.', ''),
