...
try {
    ...
    $sql = "SELECT 
                DATE_FORMAT(oh_parto_fecha, '%Y-%m') as month,
                COUNT(*) as birth_count,
                AVG(oh_parto_numero) as avg_birth_number,
                MAX(oh_parto_numero) as max_birth_number,
                COUNT(*) as record_count
            FROM oh_parto 
            WHERE oh_parto_fecha IS NOT NULL 
            AND oh_parto_numero > 0
            GROUP BY DATE_FORMAT(oh_parto_fecha, '%Y-%m')
            ORDER BY DATE_FORMAT(oh_parto_fecha, '%Y-%m') ASC";
