...
try {
    ...
    $query = "
        SELECT 
            c.oh_concentrado_tagid,
            v.nombre as animal_nombre,
            c.oh_concentrado_racion,
            c.oh_concentrado_costo,
            c.oh_concentrado_fecha
        FROM 
            oh_concentrado c
            LEFT JOIN ovino v ON c.oh_concentrado_tagid = v.tagid 
        WHERE 
            c.oh_concentrado_fecha IS NOT NULL 
            AND c.oh_concentrado_racion > 0 
            AND c.oh_concentrado_costo > 0
        ORDER BY 
            c.oh_concentrado_fecha ASC
    ";
    ...
    foreach ($data as $row) {
        ...
        $dailyCost = (float)$row['oh_concentrado_racion'] * (float)$row['oh_concentrado_costo'];
        
        // Get date range: start date + validity period
        $startDate = new DateTime($row['oh_concentrado_fecha']);
        ...
        while ($currentDate <= $endDate) {
            ...
            $animalKey = $row['oh_concentrado_tagid'];
            ...
            if (!isset($monthlyData[$monthKey][$animalKey])) {
                $monthlyData[$monthKey][$animalKey] = array(
                    'tagid' => $row['oh_concentrado_tagid'],
    ...
    foreach ($monthlyData as $month => $animals) {
        foreach ($animals as $animalData) {
            $formattedData[] = array(
                ...
                'oh_concentrado_cantidad' => round($animalData['total_cost'], 2),
