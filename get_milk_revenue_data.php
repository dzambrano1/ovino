...
try {
    ...
    $sql = "SELECT 
                ...
                oh_leche_fecha_fin,
                CASE 
                    WHEN oh_leche_fecha_fin IS NOT NULL AND oh_leche_fecha_inicio IS NOT NULL 
                    THEN DATEDIFF(oh_leche_fecha_fin, oh_leche_fecha_inicio) + 1
                    ELSE 1
                END as period_days
            FROM oh_leche l
            LEFT JOIN ovino v ON l.oh_leche_tagid = v.tagid
            WHERE oh_leche_fecha_inicio IS NOT NULL 
            AND oh_leche_peso > 0 
            AND oh_leche_precio > 0
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $dailyMilkWeight = (float)$row['oh_leche_peso']; // Daily milk production in kg
            $pricePerKg = (float)$row['oh_leche_precio']; // Price per kg
            $dailyRevenue = $dailyMilkWeight * $pricePerKg; // Daily revenue
            
            // Get date range
            $startDate = new DateTime($row['oh_leche_fecha_inicio']);
            $endDate = $row['oh_leche_fecha_fin'] ? new DateTime($row['oh_leche_fecha_fin']) : clone $startDate;
            ...
            while ($currentDate <= $endDate) {
                ...
                if (!in_array($row['oh_leche_tagid'], $monthlyData[$monthKey]['tagids'])) {
                    $monthlyData[$monthKey]['tagids'][] = $row['oh_leche_tagid'];
                }
                
                // Track unique records for proper averaging (avoid double counting)
                $recordKey = $row['oh_leche_tagid'] . '_' . $row['oh_leche_fecha_inicio'] . '_' . $row['oh_leche_fecha_fin'];
