...
try {
    ...
    $sql = "
        ...
        FROM oh_concentrado
        WHERE oh_concentrado_racion > 0 AND oh_concentrado_costo > 0 -- Consider only valid cost entries
        ORDER BY oh_concentrado_tagid, oh_concentrado_fecha ASC;
    ";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $fecha = $row['oh_concentrado_fecha'];
            $daily_cost = (float)$row['oh_concentrado_racion'] * (float)$row['oh_concentrado_costo'];
