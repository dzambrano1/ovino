...
try {
    ...
    $sql = "
        SELECT
            ...
            oh_melaza_costo
        FROM oh_melaza
        WHERE oh_melaza_racion > 0 AND oh_melaza_costo > 0 -- Consider only valid cost entries
        ORDER BY oh_melaza_tagid, oh_melaza_fecha ASC;
    ";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            $tagid = $row['oh_melaza_tagid'];
            $fecha = $row['oh_melaza_fecha'];
            $daily_cost = (float)$row['oh_melaza_racion'] * (float)$row['oh_melaza_costo'];
