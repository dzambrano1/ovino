...
try {
    ...
    // Each oh_leche_peso is treated as daily average production for that month
    // Formula: daily_average × days_in_month × price_per_unit = monthly_revenue
    $query = "
        SELECT 
            DATE_FORMAT(oh_leche_fecha, '%Y-%m') AS month,
            ROUND(SUM(
                oh_leche_peso * 
                DAY(LAST_DAY(oh_leche_fecha)) * 
                oh_leche_precio
            ), 2) AS total_milk_value,
            UNIX_TIMESTAMP(MIN(oh_leche_fecha)) as month_timestamp
        FROM oh_leche 
        WHERE oh_leche_peso > 0 AND oh_leche_precio > 0 
        GROUP BY DATE_FORMAT(oh_leche_fecha, '%Y-%m')
        ORDER BY month ASC
    ";
