...
try {
    ...
    $sql = "
        SELECT
            ...
            oh_sal_costo
        FROM oh_sal
        WHERE oh_sal_racion > 0 AND oh_sal_costo > 0 -- Consider only valid cost entries
        ORDER BY oh_sal_tagid, oh_sal_fecha ASC;
    ";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            $tagid = $row['oh_sal_tagid'];
            $fecha = $row['oh_sal_fecha'];
            $daily_cost = (float)$row['oh_sal_racion'] * (float)$row['oh_sal_costo'];
