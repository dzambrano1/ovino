...
try {
    ...
    $sql = "SELECT 
                ...
                SUM(oh_peso_animal * oh_peso_precio) as total_revenue,
                SUM(oh_peso_animal) as total_quantity,
                AVG(oh_peso_precio) as avg_price,
                COUNT(*) as record_count
            FROM oh_peso 
            WHERE oh_peso_fecha IS NOT NULL 
            AND oh_peso_animal > 0 
            AND oh_peso_precio > 0
            GROUP BY DATE_FORMAT(oh_peso_fecha, '%Y-%m')
            ORDER BY DATE_FORMAT(oh_peso_fecha, '%Y-%m') ASC";
