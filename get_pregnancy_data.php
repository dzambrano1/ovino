...
try {
    ...
    $sql = "SELECT 
                DATE_FORMAT(oh_gestacion_fecha, '%Y-%m') as month,
                COUNT(*) as pregnancy_count,
                AVG(oh_gestacion_numero) as avg_pregnancy_number,
                MAX(oh_gestacion_numero) as max_pregnancy_number,
                COUNT(*) as record_count
            FROM oh_gestacion 
            WHERE oh_gestacion_fecha IS NOT NULL 
            AND oh_gestacion_numero > 0
            GROUP BY DATE_FORMAT(oh_gestacion_fecha, '%Y-%m')
            ORDER BY DATE_FORMAT(oh_gestacion_fecha, '%Y-%m') ASC";
