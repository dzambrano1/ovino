...
try {
    ...
    if ($configResult && mysqli_num_rows($configResult) > 0) {
        ...
        $validityDays = intval($configRow['o_vencimiento_sal']);
    }
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $startDate = new DateTime($row['oh_sal_fecha_inicio']);
            $endDate = new DateTime($row['oh_sal_fecha_fin']);
            ...
            $totalExpense = (float)$row['oh_sal_racion'] * (float)$row['oh_sal_costo'] * $totalDays;
            ...
            while ($currentDate <= $endDate) {
                ...
                $monthlyData[$monthKey]['total_quantity'] += (float)$row['oh_sal_racion'];
                $monthlyData[$monthKey]['total_cost_sum'] += (float)$row['oh_sal_costo'];
