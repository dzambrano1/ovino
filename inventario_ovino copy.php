...
require_once './pdo_conexion.php';  // Go up one directory since inventario_ovino.php is in the ovino folder
...
$sql = "SELECT id, tagid, nombre, genero, raza, etapa, grupo, estatus, fecha_nacimiento, image, image2, image3, video 
        FROM ovino" . $where_clause . " ORDER BY tagid ASC";
...
// Sexo Counts
$sexoCountsQuery = "SELECT genero, COUNT(*) as count FROM ovino" . $where_clause . " GROUP BY genero";
...
// Raza Counts
$razaCountsQuery = "SELECT raza, COUNT(*) as count FROM ovino" . $where_clause . " GROUP BY raza";
...
// Etapa Counts
$etapaCountsQuery = "SELECT etapa, COUNT(*) as count FROM ovino" . $where_clause . " GROUP BY etapa";
...
// Grupos Counts
$grupoCountsQuery = "SELECT grupo, COUNT(*) as count FROM ovino" . $where_clause . " GROUP BY grupo";
...
// Estatus Counts
$estatusCountsQuery = "SELECT estatus, COUNT(*) as count FROM ovino" . $where_clause . " GROUP BY estatus";
...
$tagidsQuery = "SELECT tagid FROM ovino" . $where_clause;
...
$avgPesoQuery = "
    SELECT 
        DATE_FORMAT(oh_peso_fecha, '%Y-%m') AS peso_month, 
        AVG(oh_peso_animal) AS average_weight 
    FROM 
        oh_peso 
    WHERE 
        oh_peso_tagid IN (" . implode(',', $tagids) . ") 
    GROUP BY 
        peso_month 
    ORDER BY 
        peso_month ASC
...
$avgLecheQuery = "
    SELECT 
        DATE_FORMAT(oh_leche_fecha_inicio, '%Y-%m') AS leche_month, 
        AVG(oh_leche_peso) AS average_leche 
    FROM 
        oh_leche
    WHERE 
        oh_leche_tagid IN (" . implode(',', $tagids) . ") 
    GROUP BY 
        leche_month 
    ORDER BY 
        leche_month ASC
...
// Fetch average monthly oh_concentrado_racion * oh_concentrado_costo from oh_concentrado table
$avgRacionQuery = "
    SELECT 
        DATE_FORMAT(oh_concentrado_fecha_inicio, '%Y-%m') AS racion_month,
        AVG(oh_concentrado_racion * oh_concentrado_costo) AS average_racion_cost
    FROM 
        oh_concentrado
    GROUP BY 
        racion_month
...
// Fetch unique tagids from oc__historicos_tareas_partos
$tagidQuery = "SELECT DISTINCT oh_parto_tagid FROM oh_parto";
$tagidResult = $conn->query($tagidQuery);

$tagids = [];
if ($tagidResult && $tagidResult->num_rows > 0) {
    while ($row = $tagidResult->fetch_assoc()) {
        $tagids[] = $row['oh_parto_tagid'];
    }
}
...
$partoQuery = "
     SELECT 
         DATE_FORMAT(oh_parto_fecha, '%Y-%m') AS yearmonth,
         oh_parto_tagid,
         SUM(oh_parto_numero) AS total_parto
     FROM 
         oh_parto
     GROUP BY 
         DATE_FORMAT(oh_parto_fecha, '%Y-%m'), oh_parto_tagid
     ORDER BY 
         DATE_FORMAT(oh_parto_fecha, '%Y-%m') ASC
";
...
<script>
    ...
    function registrar(tagid) {
        if (confirm('¿Desea generar un informe completo para este animal?')) {
            ...
            window.location.href = 'ovino_report.php?tagid=' + encodeURIComponent(tagid);
        }
    }
</script>
<script>
    ...
    function openUpdateModal(tagid) {
        ...
        console.log('Starting AJAX request to fetch_ovino_data.php');
        
        $.ajax({
            url: 'fetch_ovino_data.php',
            type: 'GET',
            data: { tagid: tagid },
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <select class="form-select" name="raza" id="updateRaza" required>
                                            ...
                                            $sql_razas = "SELECT DISTINCT oc_razas_nombre FROM oc_razas ORDER BY oc_razas_nombre";
                                            $result_razas = $conn_razas->query($sql_razas);
                                            while ($row_razas = $result_razas->fetch_assoc()) {
                                                echo '<option value="' . htmlspecialchars($row_razas['oc_razas_nombre']) . '">' 
                                                    . htmlspecialchars($row_razas['oc_razas_nombre']) . '</option>';
                                            }
                                            $conn_razas->close();
                                            ?>
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <select class="form-select" name="etapa" id="updateEtapa" required>
                                            ...
                                            $sql_etapa = "SELECT DISTINCT oc_etapas_nombre FROM oc_etapas ORDER BY oc_etapas_nombre";
                                            $result_etapa = $conn_etapa->query($sql_etapa);
                                            while ($row_etapa = $result_etapa->fetch_assoc()) {
                                                echo '<option value="' . htmlspecialchars($row_etapa['oc_etapas_nombre']) . '">' 
                                                    . htmlspecialchars($row_etapa['oc_etapas_nombre']) . '</option>';
                                            }
                                            $conn_etapa->close();
                                            ?>
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <select class="form-select" name="grupo" id="updateGrupo" required>
                                            ...
                                            $sql_grupos = "SELECT DISTINCT oc_grupos_nombre FROM oc_grupos ORDER BY oc_grupos_nombre";
                                            $result_grupos = $conn_grupos->query($sql_grupos);
                                            while ($row_grupos = $result_grupos->fetch_assoc()) {
                                                echo '<option value="' . htmlspecialchars($row_grupos['oc_grupos_nombre']) . '">' 
                                                    . htmlspecialchars($row_grupos['oc_grupos_nombre']) . '</option>';
                                            }
                                            $conn_grupos->close();
                                            ?>
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <select class="form-select" name="estatus" id="updateEstatus" required>
                                            ...
                                            $sql_estatus = "SELECT DISTINCT oc_estatus_nombre FROM oc_estatus ORDER BY oc_estatus_nombre";
                                            $result_estatus = $conn_estatus->query($sql_estatus);
                                            while ($row_estatus = $result_estatus->fetch_assoc()) {
                                                echo '<option value="' . htmlspecialchars($row_estatus['oc_estatus_nombre']) . '">' 
                                                    . htmlspecialchars($row_estatus['oc_estatus_nombre']) . '</option>';
                                            }
                                            $conn_estatus->close();
                                            ?>
                                        </select>
...
async function generateAnimalPDF(tagid) {
    try {
        ...
        const response = await fetch(`ovino_report.php?tagid=${encodeURIComponent(tagid)}`, {
            headers: {
...
async function sendChatMessage(mensaje) {
    try {
        ...
        let promptPrefix = `Como veterinario especializado en ganado ovino, analiza el historial médico y responde: `;
        
        if (mensaje === 'Produccion carnica') {
            promptPrefix = `Como veterinario especializado en ganado ovino, muestra todos los registros de la tabla Produccion Carnica y responde sobre las tendencias estadísticas del peso del rebaño. IMPORTANTE: No menciones números de tag ID en tu respuesta, ya que es una estadística general de toda la finca, no de un animal específico. Responde: `;
        } else if (mensaje === 'Produccion lechera') {
            promptPrefix = `Como veterinario especializado en ganado ovino, muestra todos los registros de la tabla Produccion Lechera y analiza la tendencia de la produccion de toda la finca y responde sobre las estadísticas de producción de leche. IMPORTANTE: No menciones números de tag ID en tu respuesta, ya que es una estadística general de toda la finca, no de un animal específico. Responde: `;
        } else if (mensaje === 'Consumo Concentrado') {
            promptPrefix = `Como veterinario especializado en ganado ovino, muestra todos los registros de la tabla Consumo Concentrado y analiza el historial de toda la finca y responde sobre las estadísticas de consumo de alimento concentrado. IMPORTANTE: No menciones números de tag ID en tu respuesta, ya que es una estadística general de toda la finca, no de un animal específico. Responde: `;
        } else if (mensaje === 'Conversion') {
            promptPrefix = `Como veterinario especializado en ganado ovino, analiza el historial de la tabla de conversion de toda la finca y responde sobre el índice de conversión alimenticia. IMPORTANTE: No menciones números de tag ID en tu respuesta, ya que es una estadística general de toda la finca, no de un animal específico. Responde: `;
        } else if (mensaje === 'Vacunas') {
            promptPrefix = `Como veterinario especializado en ganado ovino, analiza el historial de vacunas de aftosa, brucelosis, clostridiosis, neumonia, ectima, parasitos, garrapatas, mastitis de toda la finca y responde sobre el resumen de vacunaciones y tratamientos. IMPORTANTE: No menciones números de tag ID en tu respuesta, ya que es una estadística general de toda la finca, no de un animal específico. Responde: `;
        } else if (mensaje === 'Preñez') {
            promptPrefix = `Como veterinario especializado en ganado ovino, analiza el historial de la tabla de preñez de toda la finca y responde sobre las estadísticas de preñez. IMPORTANTE: No menciones números de tag ID en tu respuesta, ya que es una estadística general de toda la finca, no de un animal específico. Responde: `;
        } else if (mensaje === 'Dias Abiertos') {
            promptPrefix = `Como veterinario especializado en ganado ovino, analiza el historial de la tabla de Dias Abiertos de toda la finca y responde sobre las estadísticas de días abiertos. IMPORTANTE: No menciones números de tag ID en tu respuesta, ya que es una estadística general de toda la finca, no de un animal específico. Responde: `;
        } else if (mensaje === 'Descartes') {
            promptPrefix = `Como veterinario especializado en ganado ovino, analiza el historial médico de la tabla Descartes en base a los vientres con mas de 365 dias sin gestar en toda la finca y responde sobre las estadísticas de hembras sin registro de gestación. IMPORTANTE: No menciones números de tag ID en tu respuesta, ya que es una estadística general de toda la finca, no de un animal específico. Responde: `;
        } else if (mensaje === 'sin parir') {
            promptPrefix = `Como veterinario especializado en ganado ovino, analiza el historial médico de toda la finca y responde sobre las estadísticas de animales con más de 365 días sin parir. IMPORTANTE: No menciones números de tag ID en tu respuesta, ya que es una estadística general de toda la finca, no de un animal específico. Responde: `;
        } else if (mensaje === 'Consumo de Melaza') {
            promptPrefix = `Como veterinario especializado en ganado ovino, analiza el historial de consumo de melaza de toda la finca y responde sobre las estadísticas de consumo de melaza. IMPORTANTE: No menciones números de tag ID en tu respuesta, ya que es una estadística general de toda la finca, no de un animal específico. Responde: `;
        }
