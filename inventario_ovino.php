...
$avgPesoQuery = "
    ...
    FROM
        oh_peso
    WHERE
        oh_peso_tagid IN (" . str_repeat('?,', count($tagids) - 1) . "?)
    GROUP BY
        peso_month
    ORDER BY
        peso_month ASC
...
// Fetch unique tagids from oc_historicos_tareas_partos
...
window.generateAndUploadPDF = async function(tagid) {
    ...
    Swal.fire({
        title: 'Preparando Chat Veterinario...',
        text: 'Generando reporte médico y conectando con IA',
    ...
    // Make AJAX request to generate and upload PDF using ovino_report.php
    $.ajax({
        url: 'ovino_report.php',
        type: 'GET',
        data: {
            tagid: tagid,
            upload_to_chatpdf: '1'
        ...
        success: function(response) {
            ...
            if (response.success) {
                // Check if upload was successful and we have a sourceId
                if (response.upload_result && response.upload_result.success && response.upload_result.sourceId) {
                    // Show brief success toast and immediately open chat
                    Swal.fire({
                        icon: 'success',
                        title: '¡PDF Subido a IA!',
                    ...
                    // If upload failed, show error and offer to view PDF
                    Swal.fire({
                        icon: 'warning',
                        title: 'PDF Generado',
                        text: 'El PDF se generó correctamente, pero no se pudo subir a la IA. ¿Deseas ver el PDF?',
                        ...
                        if (result.isConfirmed) {
                            // Open PDF directly
                            const directUrl = window.location.origin + window.location.pathname.replace('inventario_ovino.php', '') + 'reports/' + response.filename;
                            const viewerUrl = window.location.origin + window.location.pathname.replace('inventario_ovino.php', '') + 'view_pdf.php?file=' + response.filename;
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <select class="form-select" name="etapa" id="updateEtapa" required>
                                            <option value="">Seleccionar</option>
                                            <?php
                                            $sql_etapa = "SELECT DISTINCT etapa FROM ovino ORDER BY etapa";
