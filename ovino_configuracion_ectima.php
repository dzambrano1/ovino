...
<title>Ovino Configuracion Ectima</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
  ...
  CONFIGURACION VACUNAS ECTIMA
  </h3>
</div> 
<!-- New Entry Modal Configuracion Ectima -->

<!-- Add New Vacuna Ectima Button -->
<div class="container my-3 text-center">
  <button type="button" class="btn btn-success text-center" data-bs-toggle="modal" data-bs-target="#newEntryModal">
    <i class="fas fa-plus-circle me-2"></i>Nueva Vacuna Ectima
  </button>
</div>
...
<div class="modal fade" id="newEntryModal" tabindex="-1" aria-labelledby="newEntryModalLabel">
      ...
      <div class="modal-content">
          <div class="modal-header">
              <h5 class="modal-title" id="newEntryModalLabel">
                  <i class="fas fa-plus-circle me-2"></i>Configurar Nueva Vacuna Ectima
              </h5>
  ...
  <!-- DataTable for ov_ectima records -->
...
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="ectimaTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                      $ectimaQuery = "SELECT * FROM oc_ectima";
                      ...
                      if (empty($ectimasData)) {
                          ...
                          foreach ($ectimasData as $row) {
                              ...
                              echo '        <button class="btn btn-warning btn-sm edit-ectima" 
                                              ...
                                              data-vacuna="' . htmlspecialchars($row['oc_ectima_vacuna'] ?? '') . '" 
                                              data-dosis="' . htmlspecialchars($row['oc_ectima_dosis'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['oc_ectima_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['oc_ectima_vigencia'] ?? '') . '"
                                              title="Editar Configuracion Vacuna Ectima">
                                              <i class="fas fa-edit"></i>
                                          </button>';
                              echo '        <button class="btn btn-danger btn-sm delete-ectima" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              title="Eliminar Configuracion Vacuna Ectima">
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['oc_ectima_vacuna'] ?? '') . "</td>";
                              
                              // Column 2: Dosis
                              echo "<td>" . htmlspecialchars($row['oc_ectima_dosis'] ?? 'N/A') . "</td>";
                              
                              // Column 3: Costo
                              echo "<td>" . htmlspecialchars($row['oc_ectima_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Vigencia
                              echo "<td>" . htmlspecialchars($row['oc_ectima_vigencia'] ?? 'N/A') . "</td>";
...
$(document).ready(function() {
    ...
    $('#saveNewEctima').click(function() {
        ...
        Swal.fire({
            ...
            text: `¿Desea registrar la dosis de ectima ${formData.dosis} ml ?`,
            icon: 'question',
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'process_configuracion_ectima.php',
                    type: 'POST',
                    data: {
                        action: 'insert',
    ...
    $('.edit-ectima').click(function() {
        ...
        // Edit Configuracion Ectima Modal dialog for editing
        var modalHtml = `
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editEctimaModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacuna Ectima
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
        ...
        $('#saveEditEctima').click(function() {
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la configuracion de ectima?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-ectima').click(function() {
        ...
        Swal.fire({
            ...
            text: `¿Está seguro de que desea eliminar la configuracion de ectima? Esta acción no se puede deshacer.`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
