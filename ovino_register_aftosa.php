...
<title>Ovino - Registro de Vacunación Aftosa</title>
<!-- Link to the Favicon -->
    ...
    <div class="container-fluid">
        <div class="row w-100">
            <div class="col-12 d-flex justify-content-center align-items-center">
                ...
                <i class="fas fa-syringe me-2"></i>REGISTROS DE AFTOSA<span class="ms-2"><i class="fas fa-shield-virus"></i></span>
                </h1>
            </div>
        </div>
    </div>
...
<div class="container text-center">
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT oc_aftosa_vacuna FROM oc_aftosa ORDER BY oc_aftosa_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['oc_aftosa_vacuna']) . '">' . htmlspecialchars($vacuna_row['oc_aftosa_vacuna']) . '</option>';
                                        }
  ...
  <!-- DataTable for oh_aftosa records -->
              ...
              <tbody>
                  ...
                  try {
                      $aftosaQuery = "SELECT 
                                          ...
                                          va.oh_aftosa_producto,
                                          va.oh_aftosa_dosis,
                                          va.oh_aftosa_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.oh_aftosa_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_aftosa_history
                                      FROM 
                                          ovino v
                                      LEFT JOIN 
                                          -- Join with all aftosa records per tagid
                                          oh_aftosa va ON v.tagid = va.oh_aftosa_tagid 
                                      ORDER BY 
                                          va.oh_aftosa_fecha DESC, v.tagid ASC;";
                          ...
                          foreach ($aftosasData as $row) {
                              ...
                              if ($row['in_aftosa_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-aftosa" 
                                                  ...
                                                  data-vacuna="' . htmlspecialchars($row['oh_aftosa_producto'] ?? '') . '" 
                                                  data-dosis="' . htmlspecialchars($row['oh_aftosa_dosis'] ?? '') . '" 
                                                  data-costo="' . htmlspecialchars($row['oh_aftosa_costo'] ?? '') . '" 
                                                  data-fecha="' . htmlspecialchars($row['oh_aftosa_fecha'] ?? '') . '"
                                                  title="Editar Vacuna">
                              ...
                              echo "<td>" . htmlspecialchars($row['oh_aftosa_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_aftosa_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['oh_aftosa_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['oh_aftosa_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['oh_aftosa_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['oh_aftosa_fecha'])) {
                                          $aftosaDate = new DateTime($row['oh_aftosa_fecha']);
                                      ...
                                      error_log("Date error: " . $e->getMessage() . " for date: " . $row['oh_aftosa_fecha']);
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['oc_aftosa_vacuna']) . '">' . htmlspecialchars($vacuna_row['oc_aftosa_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
