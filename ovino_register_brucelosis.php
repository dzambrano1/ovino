...
<title>ovino - Registro de Vacunación Brucelosis</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT oc_brucelosis_vacuna FROM oc_brucelosis ORDER BY oc_brucelosis_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['oc_brucelosis_vacuna']) . '">' . htmlspecialchars($vacuna_row['oc_brucelosis_vacuna']) . '</option>';
                                        }
  ...
  <!-- DataTable for oh_brucelosis records -->
              ...
              <tbody>
                  ...
                  try {
                      $brucelosisQuery = "SELECT 
                                          ...
                                          v.tagid,
                                          v.nombre,
                                          va.oh_brucelosis_fecha,
                                          va.id AS brucelosis_record_id, -- Alias brucelosis record ID
                                          va.oh_brucelosis_producto,
                                          va.oh_brucelosis_dosis,
                                          va.oh_brucelosis_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.oh_brucelosis_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_brucelosis_history
                                      FROM 
                                          ovino v
                                      LEFT JOIN 
                                          -- Join with all brucelosis records per tagid
                                          oh_brucelosis va ON v.tagid = va.oh_brucelosis_tagid 
                                      ORDER BY 
                                          va.oh_brucelosis_fecha DESC, v.tagid ASC;";
                      ...
                      if (empty($brucelosisData)) {
                          ...
                          try {
                              $configQuery = "SELECT o_vencimiento_brucelosis FROM o_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['o_vencimiento_brucelosis'])) {
                                  $vigencia = intval($row_config['o_vencimiento_brucelosis']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($brucelosisData as $row) {
                              ...
                              if ($row['in_brucelosis_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-brucelosis" 
                                                  ...
                                                  data-vacuna="' . htmlspecialchars($row['oh_brucelosis_producto'] ?? '') . '" 
                                                  data-dosis="' . htmlspecialchars($row['oh_brucelosis_dosis'] ?? '') . '" 
                                                  data-costo="' . htmlspecialchars($row['oh_brucelosis_costo'] ?? '') . '" 
                                                  data-fecha="' . htmlspecialchars($row['oh_brucelosis_fecha'] ?? '') . '"
                                                  title="Editar Vacuna">
                              ...
                              echo "<td>" . htmlspecialchars($row['oh_brucelosis_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_brucelosis_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['oh_brucelosis_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['oh_brucelosis_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['oh_brucelosis_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['oh_brucelosis_fecha'])) {
                                          $brucelosisDate = new DateTime($row['oh_brucelosis_fecha']);
                                      ...
                                      error_log("Date error: " . $e->getMessage() . " for date: " . $row['oh_brucelosis_fecha']);
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['oc_brucelosis_vacuna']) . '">' . htmlspecialchars($vacuna_row['oc_brucelosis_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
