...
<title>Ovino - Registro de Vacunación Clostridiosis</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT oc_clostridiosis_vacuna FROM oc_clostridiosis ORDER BY oc_clostridiosis_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['oc_clostridiosis_vacuna']) . '">' . htmlspecialchars($vacuna_row['oc_clostridiosis_vacuna']) . '</option>';
                                        }
  ...
  <!-- DataTable for oh_clostridiosis records -->
              ...
              <tbody>
                  ...
                  try {
                      $clostridiosisQuery = "SELECT 
                                          ...
                                          v.tagid,
                                          v.nombre,
                                          va.oh_clostridiosis_fecha,
                                          va.id AS clostridiosis_record_id, -- Alias clostridiosis record ID
                                          va.oh_clostridiosis_producto,
                                          va.oh_clostridiosis_dosis,
                                          va.oh_clostridiosis_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.oh_clostridiosis_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_clostridiosis_history
                                      FROM 
                                          ovino v
                                      LEFT JOIN 
                                          -- Join with all clostridiosis records per tagid
                                          oh_clostridiosis va ON v.tagid = va.oh_clostridiosis_tagid 
                                      ORDER BY 
                                          va.oh_clostridiosis_fecha DESC, v.tagid ASC;";
                      ...
                      if (empty($clostridiosissData)) {
                          ...
                          try {
                              $configQuery = "SELECT e_vencimiento_clostridiosis FROM e_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['e_vencimiento_clostridiosis'])) {
                                  $vigencia = intval($row_config['e_vencimiento_clostridiosis']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($clostridiosissData as $row) {
                              ...
                              if ($row['in_clostridiosis_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-clostridiosis" 
                                                  ...
                                                  data-vacuna="' . htmlspecialchars($row['oh_clostridiosis_producto'] ?? '') . '" 
                                                  data-dosis="' . htmlspecialchars($row['oh_clostridiosis_dosis'] ?? '') . '" 
                                                  data-costo="' . htmlspecialchars($row['oh_clostridiosis_costo'] ?? '') . '" 
                                                  data-fecha="' . htmlspecialchars($row['oh_clostridiosis_fecha'] ?? '') . '"
                                                  title="Editar Vacuna">
                              ...
                              echo "<td>" . htmlspecialchars($row['oh_clostridiosis_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_clostridiosis_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['oh_clostridiosis_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['oh_clostridiosis_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['oh_clostridiosis_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['oh_clostridiosis_fecha'])) {
                                          $clostridiosisDate = new DateTime($row['oh_clostridiosis_fecha']);
                                      ...
                                      error_log("Date error: " . $e->getMessage() . " for date: " . $row['oh_clostridiosis_fecha']);
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['oc_clostridiosis_vacuna']) . '">' . htmlspecialchars($vacuna_row['oc_clostridiosis_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
...
<!-- Pie Chart Section -->
<div class="container mt-5 mb-5">
    <h4 class="text-center text-black mb-4">Estado de Vacunación Clostridiosis por Grupos</h4>
