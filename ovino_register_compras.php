...
try {
    // Fetch all purchases ordered by date from ovino table
...
<title>Ovino - Registro de Compras</title>
<!-- Link to the Favicon -->
...
document.getElementById('updateModal').addEventListener('shown.bs.modal', function () {
  ...
  <!-- DataTable for ovino_compra records -->
      ...
      <div class="table-responsive">
          <table id="compraTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                  try {
                      // Query to get purchase data directly from ovino table
                      $compraQuery = "SELECT 
                                        id, 
                  ...
                  foreach ($compraData as $row) {
                      ...
                      // Render row using ovino table columns
                      echo "<tr>";
                      ...
                      echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre'] ?? 'N/A') . "</td>"; // ovino.nombre                      
                      echo "<td class='text-center'>" . htmlspecialchars($row['fecha_compra_formatted'] ?? 'N/A') . "</td>"; // Use formatted fecha_compra
                      echo "<td class='text-center'>" . htmlspecialchars($row['tagid'] ?? '') . "</td>"; // ovino.tagid
                      echo "<td class='text-center'>" . htmlspecialchars($row['precio_compra'] ?? '0.00') . "</td>"; // ovino.precio_compra
                      echo "<td class='text-center'>" . htmlspecialchars($row['peso_compra'] ?? '0.00') . "</td>"; // ovino.peso_compra
                      echo "</tr>";
                  }
                  ?>
              </tbody>
...
$(document).ready(function() {
    ...
    $('.edit-compra').click(function() {
        ...
        $.ajax({
            url: 'ovino_get_details.php', // Assuming this endpoint exists
            type: 'GET',
            data: { tagid: tagid },
            dataType: 'json',
