...
<title>Ovino - Registro de Vacunación Ectima</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="newEntryModalLabel">
                    <i class="fas fa-plus-circle me-2"></i>Nueva Vacunacion Ectima
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="newEctimaForm">
                <div class="mb-4">
                    ...
                    <div class="mb-4">                        
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <label for="new_vacuna" class="form-label">Vacunas Ectima</label>
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    <option value="">Vacunas</option>
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT oc_ectima_vacuna FROM oc_ectima ORDER BY oc_ectima_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['oc_ectima_vacuna']) . '">' . htmlspecialchars($vacuna_row['oc_ectima_vacuna']) . '</option>';
                                        }
            ...
            <div class="modal-footer btn-group">
                ...
                <button type="button" class="btn btn-success" id="saveNewEctima">
  ...
  <!-- DataTable for oh_ectima records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="ectimaTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
              ...
              <tbody>
                  ...
                  try {
                      $ectimaQuery = "SELECT 
                                          v.id AS ovino_id, -- Alias ovino ID
                                          v.tagid,
                                          v.nombre,
                                          va.oh_ectima_fecha,
                                          va.id AS ectima_record_id, -- Alias ectima record ID
                                          va.oh_ectima_producto,
                                          va.oh_ectima_dosis,
                                          va.oh_ectima_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.oh_ectima_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_ectima_history
                                      FROM 
                                          ovino v
                                      LEFT JOIN 
                                          -- Join with all ectima records per tagid
                                          oh_ectima va ON v.tagid = va.oh_ectima_tagid 
                                      ORDER BY 
                                          va.oh_ectima_fecha DESC, v.tagid ASC;";

                      $stmt = $conn->prepare($ectimaQuery);
                      $stmt->execute();
                      $ectimasData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($ectimasData)) {
                          echo "<tr><td colspan='9' class='text-center'>No hay registros disponibles</td></tr>";
                      } else {
                          // Get vigencia setting for ectima records
                          $vigencia = 30; // Default value
                          try {
                              $configQuery = "SELECT o_vencimiento_ectima FROM o_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['o_vencimiento_ectima'])) {
                                  $vigencia = intval($row_config['o_vencimiento_ectima']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($ectimasData as $row) {
                              ...
                              echo '        <button class="btn btn-success btn-sm register-new-ectima-btn" 
                              ...
                              if ($row['in_ectima_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-ectima" 
                                                  data-id="' . htmlspecialchars($row['ectima_record_id'] ?? '') . '" 
                                                  data-tagid="' . htmlspecialchars($row['tagid'] ?? '') . '" 
                                                  data-vacuna="' . htmlspecialchars($row['oh_ectima_producto'] ?? '') . '" 
                                                  data-dosis="' . htmlspecialchars($row['oh_ectima_dosis'] ?? '') . '" 
                                                  data-costo="' . htmlspecialchars($row['oh_ectima_costo'] ?? '') . '" 
                                                  data-fecha="' . htmlspecialchars($row['oh_ectima_fecha'] ?? '') . '"
                                                  title="Editar Vacuna">
                                                  <i class="fas fa-edit"></i>
                                              </button>';
                                  echo '        <button class="btn btn-danger btn-sm delete-ectima" 
                                                  data-id="' . htmlspecialchars($row['ectima_record_id'] ?? '') . '"
                              ...
                              echo "<td>" . htmlspecialchars($row['oh_ectima_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_ectima_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['oh_ectima_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['oh_ectima_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['oh_ectima_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['oh_ectima_fecha'])) {
                                          $ectimaDate = new DateTime($row['oh_ectima_fecha']);
                                          $dueDate = clone $ectimaDate;
                                      ...
                                      error_log("Date error: " . $e->getMessage() . " for date: " . $row['oh_ectima_fecha']);
                      ...
                      error_log("Error in ectima table: " . $e->getMessage());
...
<!-- Initialize DataTable for VH ectima -->
<script>
$(document).ready(function() {
    $('#ectimaTable').DataTable({
...
$(document).ready(function() {
    ...
    // Note: editEctimaModal is created dynamically later, so no need to initialize here.

    // Handle new entry form submission
    $('#saveNewEctima').click(function() {
        // Validate the form
        var form = document.getElementById('newEctimaForm');
        ...
        Swal.fire({
            ...
            text: `¿Desea registrar la dosis de ectima ${formData.dosis} ml para el animal con Tag ID ${formData.tagid}?`,
            icon: 'question',
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            ...
                            text: 'El registro de ectima ha sido guardado correctamente',
                            icon: 'success',
                            confirmButtonColor: '#28a745'
                        }).then(() => {
    ...
    $('.edit-ectima').click(function() {
        ...
        // Edit Ectima Modal dialog for editing
        var modalHtml = `
        <div class="modal fade" id="editEctimaModal" tabindex="-1" aria-labelledby="editEctimaModalLabel">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editEctimaModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacunacion Ectima
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="editEctimaForm">
                            ...
                            <div class="mb-4">                        
                                <div class="input-group">
                                    <span class="input-group-text">
                                        <i class="fa-solid fa-syringe"></i>
                                        <label for="edit_vacuna" class="form-label">Vacunas Ectima</label>
                    ...
                    <div class="modal-footer btn-group">
                        ...
                        <button type="button" class="btn btn-success" id="saveEditEctima">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['oc_ectima_vacuna']) . '">' . htmlspecialchars($vacuna_row['oc_ectima_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
        ...
        // Show the edit modal
        var editModal = new bootstrap.Modal(document.getElementById('editEctimaModal'));
        ...
        $('#saveEditEctima').click(function() {
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar el registro de ectima para el animal con Tag ID ${formData.tagid}?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $(document).on('click', '.register-new-ectima-btn', function() { 
...
$(document).ready(function() {
    ...
    function updateChart() {
        ...
        let filteredData = [...allEctimaData];
    ...
    function createEctimaChart(data) {
        var ctx = document.getElementById('ectimaChart').getContext('2d');
...
<!-- Pie Chart Section -->
<div class="container mt-5 mb-5">
    <h4 class="text-center text-black mb-4">Estado de Vacunación Ectima por Grupos</h4>
    <div class="row justify-content-center" id="groupChartsContainer">
        <!-- Group charts will be populated dynamically here -->
    </div>
</div>

<!-- Chart.js Script for Ectima Status Pie Charts -->
<script>
$(document).ready(function() {
    const ectimaStatusUrl = 'get_ectima_status_by_grupo.php';
                ...
                <div class="card shadow">
                    ...
                    <div class="card-body">
                        <div class="chart-container" style="position: relative; height:40vh; width:100%">
                            <canvas id="ectimaPie${safeGrupo}"></canvas>
                            <div id="ectimaPie${safeGrupo}Message" class="text-center small text-muted mt-2"></div>
                        </div>
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.error) {
                console.error('Error fetching ectima status data:', data.error);
            ...
            grupos.forEach(grupo => {
                ...
                const canvasId = `ectimaPie${safeGrupo}`;
                const messageId = `ectimaPie${safeGrupo}Message`;
        ...
        error: function(xhr, status, error) {
            console.error('AJAX error fetching ectima status data:', error);
