...
try {
    ...
    $concentradoQuery = "SELECT 
                            oh_concentrado_fecha_inicio,
                            oh_concentrado_fecha_fin,
                            oh_concentrado_racion,
                            oh_concentrado_costo,
                            oh_concentrado_tagid
                        FROM oh_concentrado 
                        WHERE oh_concentrado_fecha_inicio IS NOT NULL 
                        AND oh_concentrado_fecha_fin IS NOT NULL 
                        AND oh_concentrado_racion IS NOT NULL 
                        AND oh_concentrado_costo IS NOT NULL
                        ORDER BY oh_concentrado_fecha_inicio ASC";
    ...
    if (empty($concentradoData)) {
        ...
        $concentradoQuery = "SELECT 
                                oh_concentrado_fecha,
                                oh_concentrado_fecha as oh_concentrado_fecha_inicio,
                                oh_concentrado_fecha as oh_concentrado_fecha_fin,
                                oh_concentrado_racion,
                                oh_concentrado_costo,
                                oh_concentrado_tagid
                            FROM oh_concentrado 
                            WHERE oh_concentrado_fecha IS NOT NULL 
                            AND oh_concentrado_racion IS NOT NULL 
                            AND oh_concentrado_costo IS NOT NULL
                            ORDER BY oh_concentrado_fecha ASC";
    ...
    foreach ($concentradoData as $concentrado) {
        $startDate = new DateTime($concentrado['oh_concentrado_fecha_inicio']);
        $endDate = new DateTime($concentrado['oh_concentrado_fecha_fin']);
        
        // Calculate total expense value
        $totalDays = $endDate->diff($startDate)->days + 1;
        $totalValue = $concentrado['oh_concentrado_racion'] * $concentrado['oh_concentrado_costo'] * $totalDays;
        $dailyValue = $totalValue / $totalDays;
        
        error_log("Processing concentrado: Start=" . $concentrado['oh_concentrado_fecha_inicio'] . 
                 ", End=" . $concentrado['oh_concentrado_fecha_fin'] . 
                 ", Days=" . $totalDays . 
...
<title>Ovino - Registro de Alimento Concentrado</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_alimento" name="alimento" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_alimentos = "SELECT DISTINCT oc_concentrado_nombre FROM oc_concentrado ORDER BY oc_concentrado_nombre ASC";
                                        ...
                                        foreach ($alimentos as $alimento_row) {
                                            echo '<option value="' . htmlspecialchars($alimento_row['oc_concentrado_nombre']) . '">' . htmlspecialchars($alimento_row['oc_concentrado_nombre']) . '</option>';
                                        }
                                    } catch (PDOException $e) {
  ...
  <!-- DataTable for oh_concentrado records -->
              ...
              <tbody>
                  ...
                  try {
                      // Query to get all Animals and ALL their concentrado records (if any)
                        $concentradoQuery = "
                            SELECT
                                ...
                                v.etapa AS ovino_etapa,
                                c.id AS concentrado_id,         -- Will be NULL for animals with no concentrado records
                                c.oh_concentrado_fecha_inicio,
                                c.oh_concentrado_fecha_fin,
                                c.oh_concentrado_tagid,         -- Matches ovino_tagid if concentrado exists
                                c.oh_concentrado_etapa,
                                c.oh_concentrado_producto,
                                c.oh_concentrado_racion,
                                c.oh_concentrado_costo,
                                -- Calculate total_value only if c.id is not null
                                CASE WHEN c.id IS NOT NULL THEN CAST((c.oh_concentrado_racion * c.oh_concentrado_costo) AS DECIMAL(10,2)) ELSE NULL END as total_value
                            FROM
                                ovino v
                            LEFT JOIN
                                oh_concentrado c ON v.tagid = c.oh_concentrado_tagid -- Join ALL matching concentrado records
                            ORDER BY
                                ...
                                c.oh_concentrado_fecha_inicio DESC";
                      ...
                      if (empty($concentradoData)) {
                          ...
                          try {
                              $configQuery = "SELECT o_vencimiento_concentrado FROM o_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              
                              // Explicitly use PDO fetch method
                              $row = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row && isset($row['o_vencimiento_concentrado'])) {
                                  $vigencia = intval($row['o_vencimiento_concentrado']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($concentradoData as $row) {
                              ...
                              $concentradoFecha = $row['oh_concentrado_fecha_inicio'] ?? null;
                              $concentradoFechaInicio = $row['oh_concentrado_fecha_inicio'] ?? null;
                              $concentradoFechaFin = $row['oh_concentrado_fecha_fin'] ?? null;
                              ...
                              if ($hasConcentrado) {
                                  ...
                                  echo '        <button class="btn btn-warning btn-sm edit-concentrado" 
                                                  ...
                                                  data-etapa="'.htmlspecialchars($row['ovino_etapa'] ?? '').'" 
                                                  data-producto="'.htmlspecialchars($row['oh_concentrado_producto'] ?? '').'" 
                                                  data-racion="'.htmlspecialchars($row['oh_concentrado_racion'] ?? '').'" 
                                                  data-costo="'.htmlspecialchars($row['oh_concentrado_costo'] ?? '').'" 
                                                  data-fecha-inicio="'.htmlspecialchars($concentradoFechaInicio ?? '').'"
                                                  data-fecha-fin="'.htmlspecialchars($concentradoFechaFin ?? '').'"
                                                  title="Editar Registro"
                                                  data-debug="DB: '.htmlspecialchars($row['oh_concentrado_producto'] ?? 'NULL').'">
                              ...
                              echo "<td>" . htmlspecialchars($row['ovino_etapa'] ?? 'N/A') . "</td>";
                              // Column 6: Producto (or N/A)
                              echo "<td>" . ($hasConcentrado ? htmlspecialchars($row['oh_concentrado_producto'] ?? '') : 'N/A') . "</td>";
                              // Column 7: Racion (or N/A)
                              echo "<td>" . ($hasConcentrado ? htmlspecialchars($row['oh_concentrado_racion'] ?? '') : 'N/A') . "</td>";
                              // Column 8: Costo (or N/A)
                              echo "<td>" . ($hasConcentrado ? htmlspecialchars($row['oh_concentrado_costo'] ?? '') : 'N/A') . "</td>";
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="productList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $alimentos = $stmt_alimentos->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($alimentos as $alimento_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action product-option" data-value="' . htmlspecialchars($alimento_row['oc_concentrado_nombre']) . '">' . htmlspecialchars($alimento_row['oc_concentrado_nombre']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching alimentos: " . $e->getMessage());
