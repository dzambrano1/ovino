...
$chartData = [];
try {
    $chartQuery = "SELECT DATE_FORMAT(oh_gestacion_fecha, '%Y-%m') as month_year, COUNT(*) as count 
                     FROM oh_gestacion 
                     GROUP BY month_year 
...
<title>Ovino Register Gestacion</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
  ...
  <!-- DataTable for oh_gestacion records -->
              ...
              <?php
                  try {
                      // Query to get all Female animals and their LATEST gestacion record (if any)
                        $gestacionQuery = "
                            SELECT
                                ...
                                ins.oh_gestacion_fecha,
                                ins.oh_gestacion_tagid, -- Will be NULL if no gestacion
                                ins.oh_gestacion_numero
                            FROM
                                ovino v
                            LEFT JOIN oh_gestacion ins
                              ON v.tagid = ins.oh_gestacion_tagid
                            LEFT JOIN oh_gestacion ins_check
                              ON ins.oh_gestacion_tagid = ins_check.oh_gestacion_tagid
                             AND ins.oh_gestacion_fecha < ins_check.oh_gestacion_fecha -- Check for a newer record
                            ...
                            ORDER BY
                                ...
                                ins.oh_gestacion_fecha DESC, -- Then by latest insemination date
                      ...
                      if (empty($gestacionData)) {
                          ...
                          try {
                              $configQuery = "SELECT o_vencimiento_gestacion FROM o_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              
                              // Explicitly use PDO fetch method
                              $row = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row && isset($row['o_vencimiento_gestacion'])) {
                                  $vigencia = intval($row['o_vencimiento_gestacion']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($gestacionData as $row) {
                              ...
                              $gestacionFecha = $row['oh_gestacion_fecha'] ?? null;
                              ...
                              echo '<td class="text-center">
                                  <div class="btn-group" role="group">
                                      ...
                                      <button class="btn btn-warning btn-sm edit-gestacion" 
                                          ...
                                          data-numero="'.htmlspecialchars($row['oh_gestacion_numero'] ?? '').'" 
                              ...
                              echo "<td class='text-center'>" . ($hasgestacion ? htmlspecialchars($row['oh_gestacion_numero'] ?? '') : 'N/A') . "</td>";                              
