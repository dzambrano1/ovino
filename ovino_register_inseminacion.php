...
$chartData = [];
try {
    $chartQuery = "SELECT DATE_FORMAT(oh_inseminacion_fecha, '%Y-%m') as month_year, COUNT(*) as count 
...
<title>Ovino Register Inseminacion</title>
<!-- Link to the Favicon -->
                  ...
                  try {
                      // Query to get all Female animals and their LATEST insemination record (if any)
                        $inseminacionQuery = "
                            SELECT
                                ...
                                ins.oh_inseminacion_fecha,
                                ins.oh_inseminacion_tagid, -- Will be NULL if no insemination
                                ins.oh_inseminacion_numero,
                                ins.oh_inseminacion_costo
                            FROM
                                ovino v
                            LEFT JOIN oh_inseminacion ins
                              ON v.tagid = ins.oh_inseminacion_tagid
                            LEFT JOIN oh_inseminacion ins_check
                              ON ins.oh_inseminacion_tagid = ins_check.oh_inseminacion_tagid
                             AND ins.oh_inseminacion_fecha < ins_check.oh_inseminacion_fecha -- Check for a newer record
                            ...
                            ORDER BY
                                ...
                                ins.oh_inseminacion_fecha DESC, -- Then by latest insemination date
                      ...
                      if (empty($inseminacionData)) {
                          ...
                          foreach ($inseminacionData as $row) {
                              ...
                              $inseminacionFecha = $row['oh_inseminacion_fecha'] ?? null;
                              ...
                              echo '<td class="text-center">
                                  <div class="btn-group" role="group">
                                      ...
                                      <button class="btn btn-warning btn-sm edit-inseminacion" 
                                          ...
                                          data-numero="'.htmlspecialchars($row['oh_inseminacion_numero'] ?? '').'" 
                                          data-costo="'.htmlspecialchars($row['oh_inseminacion_costo'] ?? '').'" 
                              ...
                              echo "<td class='text-center'>" . ($hasInseminacion ? htmlspecialchars($row['oh_inseminacion_numero'] ?? '') : 'N/A') . "</td>";
                              echo "<td class='text-center'>" . ($hasInseminacion ? htmlspecialchars($row['oh_inseminacion_costo'] ?? '') : 'N/A') . "</td>";
