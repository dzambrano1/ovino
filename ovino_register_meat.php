...
<title>Ovino Register Carne</title>
<!-- Link to the Favicon -->
              ...
              <tbody>
                  ...
                  try {
                      // Query to get all Animals and ALL their peso records (if any)
                        $pesoQuery = "
                            SELECT
                                ...
                                p.oh_peso_fecha,
                                p.oh_peso_tagid,         -- Matches ovino_tagid if peso exists
                                p.oh_peso_animal,
                                p.oh_peso_precio,
                                -- Calculate total_value only if p.id is not null
                                CASE WHEN p.id IS NOT NULL THEN CAST((p.oh_peso_animal * p.oh_peso_precio) AS DECIMAL(10,2)) ELSE NULL END as total_value
                            FROM
                                ovino v
                            LEFT JOIN
                                oh_peso p ON v.tagid = p.oh_peso_tagid -- Join ALL matching peso records
                            ORDER BY
                      ...
                      if (empty($pesosData)) {
                          ...
                          try {
                              $configQuery = "SELECT e_vencimiento_pesaje_animal FROM e_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              
                              // Explicitly use PDO fetch method
                              $row = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row && isset($row['e_vencimiento_pesaje_animal'])) {
                                  $vigencia = intval($row['e_vencimiento_pesaje_animal']);
                              }
                          ...
                          foreach ($pesosData as $row) {
                              ...
                              $pesoFecha = $row['oh_peso_fecha'] ?? null;
                              ...
                              if ($hasPeso) {
                                  ...
                                  echo '        <button class="btn btn-warning btn-sm edit-peso" 
                                                  ...
                                                  data-tagid="'.htmlspecialchars($row['oh_peso_tagid'] ?? '').'" 
                                                  data-peso="'.htmlspecialchars($row['oh_peso_animal'] ?? '').'" 
                                                  data-precio="'.htmlspecialchars($row['oh_peso_precio'] ?? '').'" 
                                  ...
                                  echo '        <button class="btn btn-danger btn-sm delete-peso" 
                                                  data-id="'.htmlspecialchars($row['peso_id'] ?? '').'" 
                                                  data-tagid="'.htmlspecialchars($row['oh_peso_tagid'] ?? '').'" -- Pass tagid for context
                                                  title="Eliminar Peso">
                              ...
                              // Column 5: Peso (kg)
                              echo "<td>" . ($hasPeso ? htmlspecialchars($row['oh_peso_animal'] ?? '') : 'N/A') . "</td>";
                              // Column 6: Precio ($/kg)
                              echo "<td>" . ($hasPeso ? htmlspecialchars($row['oh_peso_precio'] ?? '') : 'N/A') . "</td>";
