...
try {
    ...
    $melazaQuery = "SELECT 
                        ...
                        AND oh_melaza_fecha_fin IS NOT NULL 
                        AND oh_melaza_racion IS NOT NULL 
                        AND oh_melaza_costo IS NOT NULL
    ...
    if (empty($melazaData)) {
        ...
        $melazaQuery = "SELECT 
                                oh_melaza_fecha,
                                oh_melaza_fecha as oh_melaza_fecha_inicio,
                                oh_melaza_fecha as oh_melaza_fecha_fin,
                                oh_melaza_racion,
                                oh_melaza_costo,
                                oh_melaza_tagid
                            FROM oh_melaza 
                            WHERE oh_melaza_fecha IS NOT NULL 
                            AND oh_melaza_racion IS NOT NULL 
                            AND oh_melaza_costo IS NOT NULL
                            ORDER BY oh_melaza_fecha ASC";
    ...
    foreach ($melazaData as $melaza) {
        $startDate = new DateTime($melaza['oh_melaza_fecha_inicio']);
        $endDate = new DateTime($melaza['oh_melaza_fecha_fin']);
        
        // Calculate total expense value
        $totalDays = $endDate->diff($startDate)->days + 1;
        $totalValue = $melaza['oh_melaza_racion'] * $melaza['oh_melaza_costo'] * $totalDays;
        $dailyValue = $totalValue / $totalDays;
        
        error_log("Processing melaza: Start=" . $melaza['oh_melaza_fecha_inicio'] . 
                 ", End=" . $melaza['oh_melaza_fecha_fin'] . 
                 ", Days=" . $totalDays . 
              ...
              <tbody>
                  ...
                  try {
                      // Query to get all Animals and ALL their melaza records (if any)
                        $melazaQuery = "
                            ...
                            FROM
                                ovino v
                            LEFT JOIN
                                oh_melaza c ON v.tagid = c.oh_melaza_tagid -- Join ALL matching melaza records
                            ORDER BY
                      ...
                      if (empty($melazaData)) {
                          ...
                          try {
                              $configQuery = "SELECT o_vencimiento_melaza FROM o_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              
                              // Explicitly use PDO fetch method
                              $row = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row && isset($row['o_vencimiento_melaza'])) {
                                  $vigencia = intval($row['o_vencimiento_melaza']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($melazaData as $row) {
                              ...
                              $melazaFechaInicio = $row['oh_melaza_fecha_inicio'] ?? null;
                              $melazaFechaFin = $row['oh_melaza_fecha_fin'] ?? null;
                              ...
                              if ($hasMelaza) {
                                  ...
                                  echo '        <button class="btn btn-warning btn-sm edit-melaza" 
                                                  ...
                                                  data-debug="DB: '.htmlspecialchars($row['oh_melaza_producto'] ?? 'NULL').'">
