...
<title>ovino Register Nacimientos</title>
<!-- Link to the Favicon -->
...
<div class="container nav-icons-container">
    ...
    <div class="icon-button-container">
        <button onclick="window.location.href='./ovino_feria.php'" class="icon-button">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (createEntryForm) {
            ...
            createEntryForm.addEventListener('submit', function(event) {
                ...
                fetch('ovino_nacimientos_create.php', {
                    method: 'POST',
                    body: formData
                })
...
function saveUpdates() {
    ...
    // Send the update request
        $.ajax({
        url: 'ovino_nacimiento_update.php',
            type: 'POST',
        data: formData,
...
document.getElementById('updateModal').addEventListener('shown.bs.modal', function () {
  ...
  <!-- DataTable for oh_compra records -->
      ...
      <div class="table-responsive">
          <table id="nacimientoTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                  try {
                      // Query to get nacimiento data directly from ovino table
...
$(document).ready(function() {
    ...
    $('#saveNewNacimiento').click(function() {
        // Validate the form
        var form = document.getElementById('newNacimientoForm');
        if (!form.checkValidity()) {
            form.reportValidity();
            return;
        }
    ...
    $('.edit-nacimiento').click(function() {
        ...
        var fechaNacimiento = button.data('fecha'); // Get the purchase date
        ...
        $.ajax({
            url: 'ovino_nacimientos_get_details.php',
            type: 'GET',
            data: { tagid: tagid },
            dataType: 'json',
            success: function(animalData) {
                if (animalData && animalData.success) {
                    ...
                    $('#updateFechaNacimiento').val(fechaNacimiento); // Use the specific purchase date from the button
                    ...
                    $('#updatePeso').val(data.peso_nacimiento); // Populate peso_compra
    ...
    $('.delete-nacimiento').click(function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                // Send AJAX request to delete the ovino record
                $.ajax({
                    url: 'ovino_delete.php', // Target the ovino delete script
                    type: 'POST',
                    data: {
