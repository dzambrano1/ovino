...
<title>Ovino - Registro de Vacunación IBR</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT oc_neumonia_vacuna FROM oc_neumonia ORDER BY oc_neumonia_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['oc_neumonia_vacuna']) . '">' . htmlspecialchars($vacuna_row['oc_neumonia_vacuna']) . '</option>';
                                        }
  ...
  <!-- DataTable for oh_neumonia records -->
              ...
              <tbody>
                  ...
                  try {
                      $neumoniaQuery = "SELECT 
                                          ...
                                          v.tagid,
                                          v.nombre,
                                          va.oh_neumonia_fecha,
                                          va.id AS neumonia_record_id, -- Alias neumonia record ID
                                          va.oh_neumonia_producto,
                                          va.oh_neumonia_dosis,
                                          va.oh_neumonia_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.oh_neumonia_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_neumonia_history
                                      FROM 
                                          ovino v
                                      LEFT JOIN 
                                          -- Join with all neumonia records per tagid
                                          oh_neumonia va ON v.tagid = va.oh_neumonia_tagid 
                                      ORDER BY 
                                          va.oh_neumonia_fecha DESC, v.tagid ASC;";
                      ...
                      if (empty($neumoniasData)) {
                          ...
                          try {
                              $configQuery = "SELECT oh_vencimiento_neumonia FROM oh_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['oh_vencimiento_neumonia'])) {
                                  $vigencia = intval($row_config['oh_vencimiento_neumonia']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($neumoniasData as $row) {
                              ...
                              if ($row['in_neumonia_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-neumonia" 
                                                  ...
                                                  data-vacuna="' . htmlspecialchars($row['oh_neumonia_producto'] ?? '') . '" 
                                                  data-dosis="' . htmlspecialchars($row['oh_neumonia_dosis'] ?? '') . '" 
                                                  data-costo="' . htmlspecialchars($row['oh_neumonia_costo'] ?? '') . '" 
                                                  data-fecha="' . htmlspecialchars($row['oh_neumonia_fecha'] ?? '') . '"
                                                  title="Editar Vacuna">
                              ...
                              echo "<td>" . htmlspecialchars($row['oh_neumonia_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_neumonia_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['oh_neumonia_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['oh_neumonia_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['oh_neumonia_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['oh_neumonia_fecha'])) {
                                          $neumoniaDate = new DateTime($row['oh_neumonia_fecha']);
                                      ...
                                      error_log("Date error: " . $e->getMessage() . " for date: " . $row['oh_neumonia_fecha']);
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['oc_neumonia_vacuna']) . '">' . htmlspecialchars($vacuna_row['oc_neumonia_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
