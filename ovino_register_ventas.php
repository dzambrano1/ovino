...
<title>Ovino Register Ventas</title>
<!-- Link to the Favicon -->
           ...
           <table id="soldAnimalsTable" class="table table-striped table-bordered">
               ...
               <tbody>
                   ...
                   try {
                     ...
                     $soldQuery = "SELECT v.*, vp.oh_peso_animal, vp.oh_peso_precio, vp.oh_peso_fecha as peso_fecha
                               FROM ovino v
                               LEFT JOIN (
                                   SELECT vp1.*
                                   FROM oh_peso vp1
                                   INNER JOIN (
                                       SELECT oh_peso_tagid, MAX(oh_peso_fecha) as max_fecha
                                       FROM oh_peso
                                       GROUP BY oh_peso_tagid
                                   ) vp2 ON vp1.oh_peso_tagid = vp2.oh_peso_tagid AND vp1.oh_peso_fecha = vp2.max_fecha
                               ) vp ON v.tagid = vp.oh_peso_tagid
                               WHERE v.estatus = 'Vendido'
                               ORDER BY vp.oh_peso_fecha DESC";                              
                     ...
                     if (empty($soldData)) {
                         ...
                         foreach ($soldData as $row) {
                             ...
                             echo "<td class='text-center'>" . htmlspecialchars($row['oh_peso_precio'] ?? 'N/A') . "</td>";
                             echo "<td class='text-center'>" . htmlspecialchars($row['oh_peso_animal'] ?? 'N/A') . "</td>";                            
                             echo "</tr>";
                         }
                     }
           ...
           <table id="availableAnimalsTable" class="table table-striped table-bordered">
               ...
               <tbody>
                   ...
                   try {
                     ...
                     $availableQuery = "SELECT v.*, vp.oh_peso_animal, vp.oh_peso_precio, vp.oh_peso_fecha as peso_fecha
                               FROM ovino v
                               LEFT JOIN (
                                   SELECT vp1.*
                                   FROM oh_peso vp1
                               ...
                               ORDER BY v.estatus DESC, vp.oh_peso_fecha DESC";                              
    ...
    $('#saveNewVenta').click(function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                // Send AJAX request to update ovino record with sale information
                $.ajax({
