...
<div class="container" id="nav-buttons">
    <div class="container nav-icons-container" id="nav-buttons">
        ...
        <button onclick="window.location.href='./inventario_ovino.php'" class="icon-button" data-tooltip="Inventario ovino">
            <img src="./images/vacas.png" alt="Inventario" class="nav-icon">
        </button>
        
        <button onclick="window.location.href='./ovino_indices.php'" class="icon-button" data-tooltip="Indices ovinos">
            <img src="./images/fondo-indexado.png" alt="Inicio" class="nav-icon">
        </button>
        
        <button onclick="window.location.href='./ovino_configuracion.php'" class="icon-button" data-tooltip="Configurar Tablas">
...
<div class="container scroll-icons-container">
    <button class="btn btn-outline-secondary mb-3" type="button" onclick="window.location.href='./ovino_registrar_pesaje_animal.php'" data-tooltip="Pesaje Carne">
    ...
    <button class="btn btn-outline-secondary mb-3" type="button" onclick="window.location.href='./ovino_registrar_alimentacion.php'" data-tooltip="Alimentacion">
        <img src="./images/harina-de-trigo.png" alt="Alimentacion" class="nav-icon">
    </button>

    <button class="btn btn-outline-secondary mb-3" type="button" onclick="window.location.href='./ovino_registrar_vacunacion.php'" data-tooltip="Salud">
        <img src="./images/proteger.png" alt="Salud" class="nav-icon">
    </button>
       
    <button class="btn btn-outline-secondary mb-3" type="button" onclick="window.location.href='./ovino_registrar_reproduccion.php'" data-tooltip="Reproducion">
        <img src="./images/el-embarazo.png" alt="Reproduccion" class="nav-icon">
    </button>

    <button class="btn btn-outline-secondary mb-3" type="button" data-bs-toggle="collapse" onclick="window.location.href='./ovino_registrar_poblacion.php'" data-tooltip="Poblacion">
...
<a href="./inventario_ovino.php" class="back-btn">
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM oh_leche WHERE oh_leche_tagid = ? ORDER BY oh_leche_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM oh_leche ORDER BY oh_leche_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($lecheData as $row) {
        $date = new DateTime($row['oh_leche_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['oh_leche_peso']);
        $lecheFechaLabels[] = $row['oh_leche_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['oh_leche_precio']);
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['oh_leche_peso']) * floatval($row['oh_leche_precio']);
...
<?php
    ...
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $tagid = $_GET['search'];
        $stmt = $conn->prepare("SELECT nombre FROM ovino WHERE tagid = ?");
        $stmt->execute([$tagid]);
        if ($row = $stmt->fetch()) {
    ...
    $lecheQuery = "SELECT p.*, 
                         ...
                         CAST((p.oh_leche_peso * p.oh_leche_precio) AS DECIMAL(10,2)) as total_value
                  FROM oh_leche p 
                  LEFT JOIN ovino v ON p.oh_leche_tagid = v.tagid 
                  ORDER BY p.oh_leche_fecha DESC";
                ...
                .then(data => {
                    if (data.success) {
                        Swal.fire({
                            ...
                            window.location.href = data.redirect || 'ovino_registrar_leche.php';
                        });
                    } else {
...
foreach ($lechesResult as $row) {
    if ($row['oh_leche_tagid'] == '3000') {
        error_log("Debug - TagID 3000:");
        error_log("leche_animal: " . $row['oh_leche_peso']);
        error_log("leche_precio: " . $row['oh_leche_precio']);
        error_log("total_value: " . $row['total_value']);
    }
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['oh_leche_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['oh_leche_peso']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['oh_leche_precio']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['oh_leche_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-leche'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-leche='" . htmlspecialchars($row['oh_leche_peso']) . "'
                    data-precio='" . htmlspecialchars($row['oh_leche_precio']) . "'
                    data-fecha='" . htmlspecialchars($row['oh_leche_fecha']) . "'
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        Swal.fire({
                            ...
                            window.location.href = data.redirect || 'ovino_historial.php';
                        });
                    } else {
