...
if (isset($_POST['action'])) {
    ...
    try {
        ...
        if ($action === 'insert') {
            ...
            // Execute query
            $stmt = $conn->prepare("INSERT INTO oh_peso (oh_peso_tagid, oh_peso_animal, oh_peso_precio, oh_peso_fecha) VALUES (?, ?, ?, ?)");
        ...
        // UPDATE operation
            ...
            // Execute query
            $stmt = $conn->prepare("UPDATE oh_peso SET oh_peso_tagid = ?, oh_peso_animal = ?, oh_peso_precio = ?, oh_peso_fecha = ? WHERE id = ?");
        ...
        // DELETE operation
            ...
            // Execute query
            $stmt = $conn->prepare("DELETE FROM oh_peso WHERE id = ?");
            $stmt->execute([$id]);
